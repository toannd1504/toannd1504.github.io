...
export const guest = (() => {
    ...
    const buildGoogleCalendar = () => {
        ...
        const data = new URLSearchParams({
            action: 'TEMPLATE',
            text: 'Đám cưới Thoan và Toàn',
            dates: `${formatDate('2026-02-07 09:15')}/${formatDate('2026-02-07 10:15')}`,
            details: 'Tiệc cưới.',
            location: 'Nhà hàng Thiên Tân Palace, Cái Dăm, Bãi Cháy, Quảng Ninh .',
            ctz: config.get('tz'),
        });
    ...
    const pageLoaded = () => {
        ...
        if (!token || token.length <= 0) {
            // Comment out these lines to keep the RSVP section with Google Form button visible
